// Article type badge definitions
const BADGE_TYPES = {
    trend:      { label: '💡 Trend',     cls: 'badge--trend'      },
    navod:      { label: '📖 Návod',     cls: 'badge--navod'      },
    zprava:     { label: '📰 Zpráva',    cls: 'badge--zprava'     },
    bezpecnost: { label: '🔒 Bezpečnost',cls: 'badge--bezpecnost' },
    nastroj:    { label: '🛠️ Nástroj',   cls: 'badge--nastroj'    },
    etika:      { label: '⚖️ Etika',     cls: 'badge--etika'      },
};
...
function getBadgeHtml(type) {
    ...
    if (!badge) return '';
    return `<span class="article-type-badge ${badge.cls}">${badge.label}</span>`;
}

function getCategoryColor(category) {
    const map = {
        'AI Lab': 'var(--color-students)',
        'AI Toolkit': 'var(--color-teachers)',
        'AI Pulse': 'var(--color-news)',
        'NotebookLM': '#34d399',
        'Návody': '#a855f7',
        'Nástroje': '#c084fc',
        'Novinky': '#60a5fa',
    };
    return map[category] || 'var(--accent-cyan)';
}
    ...
    const html = recentArticles.slice(0, 9).map(article => `
        <a href="${article.link}" class="recent-article-card">
            <div class="recent-article-meta">
                <span class="recent-article-category" style="color:${getCategoryColor(article.category)};background:transparent;">${article.category}</span>
                <span class="recent-article-date">${article.date}</span>
            </div>
            <div style="margin-bottom:0.5rem;">${getBadgeHtml(article.type)}</div>
            <h3 class="recent-article-title">${article.title}</h3>
            <p class="recent-article-desc">${article.description}</p>
            <span class="read-more" style="color:var(--accent-cyan);font-size:0.8rem;font-weight:600;margin-top:auto;padding-top:0.75rem;">Otevřít článek →</span>
        </a>
    `).join('');
